...
namespace Poof
{
	class Program_cli
	{
		...
		static public void performCommand(string[] commands)
		{
			...
			{
				// Scan library
				Program_cli.debugMsg("Scan library selected.");
				scanLibrary();
			}
			else if (commands[0].Equals(commandPrefix + "t"))
			{
				// Tag a picture
				Program_cli.debugMsg("Tag a picture selected.");
				...
				{
					Program_cli.debugMsg("Tagged " + filename + " with " + newTags[0] + "...");				//STUB
				}
				else
				{
					Program_cli.errorMsg("Error tagging picture");					//STUB
				}
			}
			else if (commands[0].Equals(commandPrefix + "h"))
			{
				// Set home directory
				Program_cli.debugMsg("Set home directory selected.");
			...
			{
				// Output the database
				Program_cli.debugMsg("output database selected.");
				outputDatabase();												//STUB
			}
			else
			{
				Program_cli.errorMsg("Poof doesn't understand that command. The list of supported commands are:");
				displayHelp();
			}
		}
